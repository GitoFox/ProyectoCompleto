...
// Ruta para leer el archivo CSV y devolver todos los datos
app.get('/encuestadores', (req, res) => {
  const results = [];

  fs.createReadStream('BackEnd/encuestadores.csv')
    .pipe(csv())
    ...
    .on('end', () => {
      ...
      if (encuestador) {
        ...
        if (!imagenPath || imagenPath === 'NA' || imagenPath === '') {
          ...
          imagenPath = 'BackEnd/img/Saludando.png';
        }
...
app.use('/img', express.static(path.join(__dirname, '../BackEnd/img')));
